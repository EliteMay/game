...
function finalStep(label, done) {
  return `<div class="progression-goal${done ? ' is-done' : ''}"><strong>${done ? '✓' : '○'}</strong><span>${label}</span></div>`;
}
...
function renderFinalChapterBlock(game) {
...
function renderPanel() {
  ...
  const rankBlock = definition ? `
    ...
    </section>` : renderFinalChapterBlock(game);
