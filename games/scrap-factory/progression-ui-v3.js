...
function reasonLabel(research) {
  ...
  if (research.reason === 'blueprint') return 'Blueprint / 探索技術が未確保';
...
function renderPanel() {
  ...
  const rankBlock = definition ? `
    ...
    <section class="progression-section progression-section--cap">
      <div class="progression-section__head"><div><span>FINAL CHAPTER / RANK-UP CAP</span><h3>Rank ${progression.progressionRank}</h3></div><strong>ACTIVE</strong></div>
      <p>Rank ${PLAYABLE_MAX_RANK}は昇格上限ですがMain Clearではありません。崩壊した研究施設の3 Lab技術をFactoryへ持ち帰り、FabricatorでExperimental部品を製造してCentral Coreを復旧し、Experimental Technologyを研究してください。</p>
    </section>`;
