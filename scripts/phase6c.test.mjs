...
assert.ok(ITEMS.autonomous_industrial_core, 'Autonomous Industrial Core must be a configured final product');
...

{
  ...
  assert.equal(buildingUnlockState(game, 'advanced_drone_port').unlocked, true, 'completed Experimental Technology must remain the compatibility source of truth');
  assert.equal(buildingUnlockState(game, 'experimental_power_system').unlocked, true);
}
...
function buildFullAutomationFixture() {
  ...
  function add(id, type, gx, gz, extra = {}) {
    ...
    const building = { id, type, x: world(gx), z: world(gz), rotation: 0, input: {}, output: {}, progress: 0, powerFuelSeconds: 0, powerStored: 0, logisticsCursor: 0, permanent: false, ...extra };
  ...
  function connect(source, target, label) {
    ...
    for (const dir of dirs) {
      ...
      if (occupied.has(key(first.gx, first.gz)) || occupied.has(key(second.gx, second.gz))) continue;
      if (key(first.gx, first.gz) === key(targetGrid.gx, targetGrid.gz) || key(second.gx, second.gz) === key(targetGrid.gx, targetGrid.gz)) continue;
      const queue = [[second]];
      const seen = new Set([key(first.gx, first.gz), key(second.gx, second.gz), key(sourceGrid.gx, sourceGrid.gz)]);
...

{
  ...
  for (const marker of ['Advanced Drone', 'Production Recipe Routing', 'Final Automation Contract', 'analyzeFinalAutomation']) assert.ok(automationUi.includes(marker), `Automation UI missing ${marker}`);
  for (const marker of ['advanced_drone_port', 'experimental_power_system', 'fabricator_core', 'assembler_circuit']) assert.ok(worldRuntime.includes(marker), `World runtime missing Phase 6-C visual marker ${marker}`);
}
